...
export interface Engine {
  /** Gets the list of any plugins that were loaded by the deprecated process of falling back to Node modules. */
  get legacyModules(): string[];

  /** Gets the list of files generated by the engine when `runGenerators` is called. */
  get files(): File[];

  /** Gets the list of all file changes detected by the engine when `compareFiles` is called. */
  get changes(): Record<string, FileStatus>;

  /** Gets the list of all errors encountered by the engine. */
  get errors(): BasketryError[];

  /** Gets the list of all violations encountered by the engine when `runParser` and `runRules` are called. */
  get violations(): Violation[];

  /** Gets the service definition produced by the engine when `runParser` is called. */
  ...
  runParser(): Promise<void>;
  runRules(): Promise<void>;
  runGenerators(): Promise<void>;
...
export class RpcEngine implements Engine {
  public static async load({
    ...
    onError,
    onViolation,
  }: { configPath: string } & EngineEvents): Promise<{
    engines: Engine[];
    ...
    for (const [absoluteConfigPath, localConfig] of configs.value) {
      engines.push(
        new RpcEngine(localConfig, absoluteConfigPath, fsPromises, {
          onError,
          onViolation,
        }),
      );
  ...
  constructor(
    ...
    this.basketryContext = {
      projectDirectory: path.dirname(absoluteConfigPath),
      configPath: absoluteConfigPath,
    };
  }

  private _legacyModules: string[] = [];
  public get legacyModules(): string[] {
    return this._legacyModules;
  }
  ...
  private readonly basketryContext: BasketryContext;
  ...
  public get changes(): Record<string, FileStatus> {
    return this._changes;
  }

  private readonly _errors: BasketryError[] = [];
  public get errors(): BasketryError[] {
    return this._errors;
  }
    ...
    const mappedViolations = violations.map((violation) => ({
      ...
      sourcePath:
        violation.sourcePath === '#'
          ? relative(this.basketryContext.projectDirectory, this.sourcePath)
          : violation.sourcePath,
    }));

    for (const violation of mappedViolations) {
      const sourceIndex =
        this.service?.sourcePaths.indexOf(violation.sourcePath) ?? 0;
  ...
  private __service: Service | undefined;
  public get service(): Service | undefined {
    return this.__service;
  }
  private set service(service: Service | undefined) {
    if (!service) {
    ...
    const relativePaths =
      service.sourcePaths?.map((p) => {
        const from = this.basketryContext.projectDirectory;
        const to = p === '#' ? this.sourcePath : p;
    ...
    this.pushErrors(...validation.errors);

    this.__service = validation.service
      ? { ...validation.service, sourcePaths: relativePaths }
      : undefined;
  }

  public get sourcePath(): string {
    return this.resolve(this.config.source);
  }

  public resolve(relativePath: string): string {
    return path.resolve(this.basketryContext.projectDirectory, relativePath);
  }

  private hasParserRun: boolean = false;
  public async runParser(): Promise<void> {
    if (this.hasParserRun) return;
    else this.hasParserRun = true;

    try {
      const sourceContent = await fsPromises.readFile(this.sourcePath, 'utf-8');

      const plugin = {
        command: this.config.parser,
        method: 'basketry.parse' as const,
      };
      ...
      const req: ParseRequest = {
        ...
        params: {
          ...
          basketry: this.basketryContext,
        },
      };

      const result = await runJsonRpc(
        plugin.command,
        req,
        this.basketryContext.configPath,
      );

      if (!result.ok) {
        // TODO: remove legacy node module fallback
        if (
          result.errors.every(
            (e) =>
              e.code === 'PLUGIN_ERROR' && e.message === 'Command not found',
          )
        ) {
          const moduleResult = await this.legacyRunParserAsNodeModule(
            plugin.command,
            sourceContent,
          );
          if (moduleResult.ok) {
            this._legacyModules.push(plugin.command);
            const { service, violations } = moduleResult.value;
            this.service = service;
            await this.pushViolations(...violations);
          } else {
            this.pushErrors(...result.errors);
            this.pushErrors(...moduleResult.errors);
          }
        } else {
          // TODO: this will be the only code path when legacy fallback is removed
          this.pushErrors(...result.errors);
        }
      } else {
        ...
        if (isErrorResponse(res)) {
          ...
          const { response, errors } = validateParseResponse(res);

          this.pushErrors(
            ...errors.map((e) => ({
              ...e,
              plugin,
              filepath: this.basketryContext.configPath,
            })),
          );

          if (response) {
            this.service = response.result.service;
            await this.pushViolations(...response.result.violations);
          }
        }
      }
    } catch (err) {
      this.pushErrors(fatal(err));
    }
  }

  /** @deprecated */
  private async legacyRunParserAsNodeModule(
    parserModule: string,
    sourceContent: string,
  ): AsyncResult<{ service: Service | undefined; violations: Violation[] }> {
    try {
      const { fn, errors } = loadModule(
        parserModule,
        this.basketryContext.configPath,
      );
      ...
      if (errors.length) {
        ...
        const parser = fn as Parser;

        const { service, violations } = await parser(
          sourceContent,
          this.sourcePath,
        );

        return Ok({ service, violations });
      } else {
        return Err(fatal(new Error('Unable to load parser module')));
      }
    } catch (err) {
  ...
  private hasRunRules: boolean = false;
  public async runRules(): Promise<void> {
    if (this.hasRunRules) return;
    else this.hasRunRules = true;

    if (!this.config.rules?.length) return;

    const rulePromises = this.config.rules.map((rule) => {
      if (typeof rule === 'string') {
        return this.runRule(rule, {});
      } else if (typeof rule.rule === 'string') {
        return this.runRule(rule.rule, rule.options);
      } else {
  ...
  private async runRule(ruleCommand: string, options: any): Promise<void> {
    if (!this.service) return;

    const plugin = {
      command: ruleCommand,
      method: 'basketry.validate' as const,
    };
    ...
    const req: ValidateRequest = {
      ...
      method: plugin.method,
      params: {
        context: { service: this.service, options },
        basketry: this.basketryContext,
      },
    };
    ...
    if (!result.ok) {
      ...
      if (
        result.errors.every(
          (e) => e.code === 'PLUGIN_ERROR' && e.message === 'Command not found',
        )
      ) {
        const moduleResult = await this.legacyRunRuleAsNodeModule(
          plugin.command,
          options,
        );
        if (moduleResult.ok) {
          this._legacyModules.push(plugin.command);
          const { violations } = moduleResult.value;
          await this.pushViolations(...violations);
        } else {
          this.pushErrors(...result.errors);
          this.pushErrors(...moduleResult.errors);
      ...
      if (isErrorResponse(res)) {
        ...
        const { response, errors } = validateValidateResponse(res);
  ...
  /** @deprecated */
  private async legacyRunRuleAsNodeModule(ruleModule: string, options: any) {
    try {
      const { fn, errors } = loadModule(
        ruleModule,
        this.basketryContext.configPath,
      ...
      if (errors.length) {
        return Err(errors);
      } else if (fn && this.service) {
        const rule = fn as Rule;

        const violations = await rule(this.service, options);

        return Ok({ violations });
      } else {
        return Err(fatal(new Error('really bad')));
      }
    } catch (err) {
      return Err(fatal(err));
    }
  ...
  private hasRunGenerators: boolean = false;
  public async runGenerators(): Promise<void> {
    if (this.hasRunGenerators) return;
    this.hasRunGenerators = true;

    if (!this.config.generators?.length) return;

    const generatorPromises = this.config.generators.map((generator) => {
      if (typeof generator === 'string') {
        return this.runGenerator(generator, this.config.options);
      } else if (typeof generator.generator === 'string') {
        const options =
          typeof generator.options === 'undefined'
            ? this.config.options
            : merge(generator.options, this.config.options ?? {});

        return this.runGenerator(generator.generator, options);
      } else {
        return Promise.resolve([]);
      }
    });

    // Map all files to be relative to the output or project director
    const files = (await Promise.all(generatorPromises))
      .flat()
      .map(({ path: p, ...rest }) => {
        return {
          path: [
            ...(
              this.config.output ?? this.basketryContext.projectDirectory
            ).split(sep),
            ...p,
          ],
          ...rest,
    ...
    this._files.push(
      ...withGitattributes(
        files,
        (this.config.output ?? this.basketryContext.projectDirectory).split(
          sep,
        ),
      ),
    );
  ...
  public async runGenerator(
    ...
    const plugin = {
      ...
      method: 'basketry.generate' as const,
    };
    ...
    if (!result.ok) {
      ...
      if (
        ...
        const moduleResult = await this.legacyRunGeneratorAsNodeModule(
          plugin.command,
          options,
        );
        if (moduleResult.ok) {
          this._legacyModules.push(plugin.command);
          return moduleResult.value.files;
        } else {
          this.pushErrors(...result.errors);
          this.pushErrors(...moduleResult.errors);
        }
      ...
      if (isErrorResponse(res)) {
        ...
        const { response, errors } = validateGenerateResponse(res);
        ...
        return response ? response.result.files : [];
      }
    }
  ...
  /** @deprecated */
  private async legacyRunGeneratorAsNodeModule(
    ...
    try {
      ...
      if (errors.length) {
        ...
        const generator = fn as Generator;

        const files = await generator(this.service, options);
  ...
  public async compareFiles(): Promise<void> {
    if (this.hasRunGenerators) {
      const removed = await getRemoved(
        this.files,
        this.basketryContext.projectDirectory.split(sep), // TODO: don't require an array
        this.fs,
      );
  ...
  private async compare(file: File): Promise<FileStatus> {
    try {
      const previous = (
        await fsPromises.readFile(this.resolve(join(...file.path)))
      ).toString();
  ...
  public async commitFiles(): Promise<void> {
        ...
        if (file) {
          if (file.path.length > 1 && status === 'added') {
            ...
            try {
              ...
              const dir = this.resolve(dirname(relativePath));
              await this.fs.mkdir(dir, { recursive: true });
            }
          }

          try {
            await this.fs.writeFile(absolutePath, file.contents);
          } catch (ex) {
            this.pushErrors({
              code: 'WRITE_ERROR',
...
/**
 * @deprecated Use RpcEngine instead.
 */
export class NodeEngine implements Engine {
  ...
  public readonly legacyModules: string[] = [];
...
type Response<
  TRequest extends ParseRequest | ValidateRequest | GenerateRequest,
> = TRequest['method'] extends 'basketry.parse'
  ? ParseResponse | ErrorResponse
  : TRequest['method'] extends 'basketry.validate'
  ? ValidateResponse | ErrorResponse
  : TRequest['method'] extends 'basketry.generate'
  ? GenerateResponse | ErrorResponse
  : never;

function isErrorResponse(
  res: ParseResponse | ValidateResponse | GenerateResponse | ErrorResponse,
): res is ErrorResponse {
  return (res as ErrorResponse) && 'error' in res;
}
...
>(
  ...
  rpcPayload: TRequest,
  configPath: string | undefined,
): AsyncResult<Response<TRequest>> {
  return new Promise<Result<Response<TRequest>>>((res) => {
    const [cmd, ...args] = command.split(' '); // NOTE: simplistic split; quoted args will break
    ...
    const fail = (err: unknown) => {
      if (!settled) {
        ...
        const basketryError: BasketryError = {
          code: 'PLUGIN_ERROR',
        ...
        res(Err(basketryError));
      }
    };
    const succeed = (value: Response<TRequest>) => {
      if (!settled) {
        settled = true;
        res(Ok(value));
      }
    ...
    // ENOENT and other spawn failures land here (before 'spawn' and often without 'close')
    proc.once('error', (err) => {
      const errorWithCode = err as NodeJS.ErrnoException;

      switch (errorWithCode.code) {
        case 'ENOENT':
          fail(new Error(`Command not found`));
          break;
        case 'EACCES':
          fail(new Error(`Permission denied`));
          break;
        case 'EISDIR':
          fail(new Error(`Path is a directory`));
          break;
        case 'EMFILE':
          fail(new Error(`Too many open files`));
          break;
        case 'ENFILE':
          fail(new Error(`File table overflow`));
          break;
        case 'ENOSPC':
          fail(new Error(`No space left on device`));
          break;
        case 'ENOMEM':
          fail(new Error(`Out of memory`));
          break;
        case 'ELOOP':
          fail(new Error(`Too many symbolic links`));
          break;
        case 'ETXTBSY':
          fail(new Error(`Text file busy`));
          break;
        default:
          fail(errorWithCode.message);
      }
    });
    ...
    proc.stdin.on('error', (err) => {
      fail(`Error writing to \`${command}\`: ${err.message}`);
    });
    ...
    // Only write after the process has actually spawned
    proc.once('spawn', () => {
      try {
        proc.stdin.write(JSON.stringify(rpcPayload));
        proc.stdin.end();
      } catch (err) {
        fail(err);
      }
    ...
    proc.once('close', (code, signal) => {
      if (settled) return; // e.g., ENOENT already handled in 'error'
      if (code === 0) {
        try {
          succeed(JSON.parse(output));
        } catch (err) {
