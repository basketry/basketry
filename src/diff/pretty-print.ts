...
export function prettyPrint(changes: ChangeInfo[]): void {
  console.log(chalk.bold.yellowBright('The diff feature is experimental!'));
  console.log(
    chalk.bold.yellowBright(
      'Bugs may be reported at https://github.com/basketry/basketry/issues',
    ),
  );
  ...
  let hightest: Uppercase<Category> | null = null;
  if (patchChanges.length) hightest = 'PATCH';
  if (minorChanges.length) hightest = 'MINOR';
  if (majorChanges.length) hightest = 'MAJOR';
  ...
  if (hightest) {
    console.log(
      chalk.bold.yellowBright(
        `Consider incrementing the '${hightest}' component of the service's semantic version. See more at https://semver.org.`,
      ),
    );
  } else {
    console.log(
      chalk.bold.greenBright(
        `The service's semantic version does not need to be changed. See more at https://semver.org.`,
      ),
    );
  }
